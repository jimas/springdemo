...
public class ReadBig {
	public static String fff = "D:\\jimasFile\\serialize/crsapi.log.20160802.log";
	...
	public   void test1() throws Exception {
		 ...
		 * 
		 */
		RandomAccessFile randomAccessFile = new RandomAccessFile(new File("D://accessFile.txt"), "rw");
		...
		for (int offset = 0; offset < inputBuffer.capacity(); offset += BUFFER_SIZE) {
			...
			randomAccessFile.writeBytes(new String(dst, 0, length,"gbk"));
//			System.out.println(new String(dst, 0, length,"gbk"));// new
	...
	public void test2() throws Exception {
		...
		while (channel.read(byteBuf) != -1) {
			...
			System.out.print(new String(bs, 0, size,"gbk"));
			byteBuf.clear();
		}
...
//		int bufSize = 1024;
		...
		FileChannel channel = new RandomAccessFile(fff, "r")
				.getChannel();
		while (channel.read(byteBuf) != -1) {
			int size = byteBuf.position();
			byteBuf.rewind();
